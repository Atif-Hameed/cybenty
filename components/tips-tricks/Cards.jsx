...
import { countResourceLikes, countResourceViewsClicks, getResourcesByCategoryId, resourceUnLikes, searchResource } from '@/services/api';
...
const Cards = () => {
            ...
            setResources((prevResources) =>
                prevResources.map((resource) =>
                    resource._id === resourceId
                        ? { ...resource, likeCount: isLiked ? currentLikeCount - 1 : currentLikeCount + 1 }
                        : resource
                )
            );
    ...
    const handleNextCard = () => {
        console.log("next:", openCardIndex, currentItems.length - 1)
    ...
    const handlePreviousCard = () => {
        console.log("prev:", openCardIndex)
        if (openCardIndex > 0) {
            setOpenCardIndex(openCardIndex - 1);
            setSelectedCardData(currentItems[openCardIndex - 1]);
        }
    ...
    useEffect(() => {
        if (actionId && (view == 'shared')) {
            ...
            const cardToOpen = resources.find(resource => resource._id === actionId);
            if (cardToOpen) {
    ...
    const prevTitle = openCardIndex > 0 ? currentItems[openCardIndex - 1]?.title : null;
    const nextTitle = openCardIndex < currentItems.length - 1 ? currentItems[openCardIndex + 1]?.title : null;
    ...
    const handleToggleFavorite = async (resourceId) => {
        ...
        try {
            ...
            const { favoriteResources, favoritedBy } = await toggleFavoriteResource(user._id, resourceId);
    ...
    useEffect(() => {
        ...
        if (currentItems.length > 0 && user && Object.keys(favoriteResources).length === 0) {
                ...
                <div className='flex justify-center flex-col items-center gap-6 sm:w-[80%] w-[90%] sm:px-0 px-2'>
                    ...
                    <div className='flex flex-col items-center gap-3'>
                        ...
                        <p className='sm:w-[85%] w-full text-center'>
                            Welcome to our Cybersecurity Tips & Tricks page! Here, you’ll find practical advice and expert tips to help you protect your digital life. Whether you’re a beginner or an experienced user, these insights will enhance your cybersecurity knowledge and practices.
                        </p>
                    </div>
                                ...
                                <div className='my-10'>
                                    <div className='grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 w-full gap-6'>
                                        {currentItems.map((e, i) => (
                                            <div key={i} className='bg-white rounded-lg group hover:shadow-lg flex flex-col' >
                                                <div onClick={() => handleOpenCard(i, e)} className='cursor-pointer relative'>
                                                    {
                                                        ExtractFirstImageUrl(e.blog) ?
                                                            <img src={ExtractFirstImageUrl(e.blog)} alt="image not available" className='rounded-lg h-[250px] w-full object-cover' />
                                                            :
                                                ...
                                                <div className='px-3 pt-3 h-full flex-1 justify-between flex flex-col'>
                                                    <div>
                                                        <h1 onClick={() => handleOpenCard(i, e)} className='text-2xl cursor-pointer font-semibold line-clamp-1 group-hover:text-orangeMain'>{e.title}</h1>
                                                    ...
                                                    <div className='border-t mt-3 border-bordered py-3 flex items-center justify-between gap-2' >
                                                        <div className='flex items-center justify-between w-full gap-2'>
                                                            ...
                                                            <div
                                                                ...
                                                                {isLoggedIn && likedResources[e._id] ? (
                                                                    <MdThumbUp className='text-blue-600 sm:text-lg' />
                                                                ) : (
                                                                    <MdOutlineThumbUp className='text-[#000000A1] sm:text-lg' />
                                                                )}
                                                                <p className='text-[#000000A1] sm:text-sm text-xs'>{e.likeCount} Likes</p>
                                                            </div>
                                                            {/* View Count Display */}
                                                            <div className='flex items-center gap-1'>
                                                                <Image alt='' src={icon3} className='sm:h-auto h-3' />
                                                                <p className='text-[#000000A1] sm:text-sm text-xs'>{e.clickCount || '0'} Views</p>
                                                            </div>

                                                            {/* Share Count Display */}
                                                            <div className='flex items-center gap-1 cursor-pointer' onClick={() => showShareResurce(i, e)}>
                                                                <Image alt='' src={shareIcon} className='h-4 w-4 ' />
                                                                <p className='text-[#000000A1] sm:text-sm text-xs'>{e.shareCount || '0'} Share</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
